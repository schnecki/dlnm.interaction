...
plotLagAt <- function(cp, filepath = NULL, title = NULL, xDist = 0.10, colours = colourPalette, legendPos = c(0.5, 0.10)) {
    ...
    dists <- map(seq(length(cp$at)), function(x) (x - 0.5 - length(cp$at) / 2) * xDist)
    minDist <- min(unlist(dists))
    maxDist <- max(unlist(dists))
    xVals <- unlist(map(dists, function(d) lags + d))
    ids <- unlist(map(seq(length(cp$at)), function(x) rep(colours[x], length(lags))))
    ...
    legendVals <- unlist(map(cp$at, function(x) paste("Lag=", round(x, 4))))

    plot <-
        data %>%
        ## ggplot(aes(lag, RR, group = 1)) +
